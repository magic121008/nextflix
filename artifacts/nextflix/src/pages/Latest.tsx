...
import { hasApiKey, getNowPlayingMovies, getOnAirTV } from "@/services/tmdb";

type LatestFilter = "movies" | "tv";

export default function Latest() {
  const [filter, setFilter] = useState<LatestFilter>("movies");
    ...
    <Layout>
      <div className="pt-24 px-4 md:px-8 lg:px-12 pb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <h1 className="text-3xl font-bold text-white">Latest</h1>
          <div className="flex gap-2">
            {(["movies", "tv"] as LatestFilter[]).map((f) => (
              <button
                key={f}
